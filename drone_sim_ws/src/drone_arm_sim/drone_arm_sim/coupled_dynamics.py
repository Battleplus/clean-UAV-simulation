...
from drone_arm_sim.model_analysis import UrdfModel, _axis_rotation, _vector
...
class CoupledArmDynamics:
    ...
    def _entries(
        ...
        entries = [
            (name, mass * self.mass_scale, center.copy(), inertia * self.mass_scale)
            for name, mass, center, inertia in self.model.inertial_entries(q)
        ]
        if payload is not None and payload.mass_kg > 0.0:
            transforms = self.model.link_transforms(q)
